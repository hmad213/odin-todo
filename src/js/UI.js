...
class DOM {
    ...
    for (let i = 0; i < projectList.length; i++) {
      for (let j = 0; j < projectList[i].todoList.length; j++) {
        todoContainer.appendChild(
          this.loadTodo(projectList, projectList[i], j),
        );
      }
    }
    ...
    for (let i = 1; i < projectList.length; i++) {
      ...
      div.addEventListener("click", (event) => {
        ...
        for (let i = 0; i < projects.length; i++) {
          if (
            event.target === projects[i] ||
            event.target === projects[i].querySelector("span")
          ) {
  ...
  loadTodo(projectList, project, index) {
    ...
    todoDiv.addEventListener("click", (event) => {
      if (
        event.target != starImg &&
        event.target != trashImg &&
  ...
  loadProjectPage(projectList, index) {
    ...
    for (let i = 0; i < projectList[index].todoList.length; i++) {
      todoContainer.appendChild(
        this.loadTodo(projectList, projectList[index], i),
      );
    }
    this.addListeners(projectList);
  }
  ...
  loadAddProjectDialog(projectList) {
    ...
    form.addEventListener("submit", (event) => {
      ...
      projectList.push(
        new Project(document.querySelector("dialog #title").value),
      );
      this.initialize(projectList);
    });
  ...
  loadAddTodoDialog(projectList) {
    ...
    if (this.loadedPage == "today") {
      document.querySelector("dialog form #date").value = format(
        new Date(),
        "yyyy-MM-dd",
      );
    } else if (this.loadedPage == "important") {
      ...
      document.querySelectorAll("dialog form #project-select option")[
        this.loadedPage
      ].selected = true;
    }
    ...
    form.addEventListener("submit", (event) => {
      ...
      projectList[project].addTodo(
        new Todo(title, description, date, priority, false),
      );
      this.reloadPage(projectList);
    });
  }
  ...
  loadTodoDetailsDialog(projectList, project, index) {
    ...
    inputs[2].lastChild.value = format(
      project.todoList[index].dueDate,
      "yyyy-MM-dd",
    );
    console.log(project.title.replace(/\s+/g, "-"));
    inputs[3].lastChild.querySelector(
  ...
  enableTodoEdit(projectList, project, index) {
    ...
    document
      .querySelector("dialog form .buttons img")
      ...
      .addEventListener("submit", (event) => {
        ...
        if (projectList[projectSelect] == project) {
          project.editTodo(
            ...
            date,
            priority,
            project.todoList[index].checked,
          );
        } else {
          projectList[projectSelect].addTodo(
            new Todo(
              title,
              description,
              date,
  ...
  addProjectRemoveListener(projectList) {
    document.querySelectorAll(".project .trash-button").forEach((event) =>
  ...
  addTodoStarListener(projectList) {
    document.querySelectorAll("#content .star-button").forEach((e) =>
      e.addEventListener("click", (event) => {
        ...
        if (this.loadedPage == "home") {
          ...
          for (let i = 0; i < projectList.length; i++) {
            for (let j = 0; j < projectList[i].todoList.length; j++) {
              if (
                isEqual(today, startOfDay(projectList[i].todoList[j].dueDate))
              ) {
                if (event.target.parentNode == todos[index]) {
                  projectList[i].todoList[j].togglePriority();
                }
  ...
  addTodoRemoveListener(projectList) {
    document.querySelectorAll("#content .trash-button").forEach((e) =>
      e.addEventListener("click", (event) => {
        let todos = document.querySelectorAll("#content .todo");
        if (this.loadedPage == "home") {
          let index = 0;
  ...
  addTodoCheckedListener(projectList) {
    document.querySelectorAll("#content .todo input").forEach((e) =>
      e.addEventListener("click", (event) => {
        let todos = document.querySelectorAll("#content .todo");
        if (this.loadedPage == "home") {
          let index = 0;
